...
export const googleUser = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const configuredAdminEmail = (process.env.ADMIN_EMAIL || "snehareddyaelijerla@gmail.com").toLowerCase().trim();
      if (user.role === "admin" && email.toLowerCase().trim() !== configuredAdminEmail) {
        user.role = "user";
        await user.save();
      }
    } else {
      const configuredAdminEmail = (process.env.ADMIN_EMAIL || "snehareddyaelijerla@gmail.com").toLowerCase().trim();
      const userRole = email.toLowerCase().trim() === configuredAdminEmail ? "admin" : "user";
      ...
      try {
        await createNotification(
          user._id,
          "Welcome to AspirePath AI! Your account has been created successfully.",
          "success"
        );
      } catch (notifError) {
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    const frontendBaseUrl = process.env.FRONTEND_URL || process.env.CLIENT_URL || "https://aspirepath-ai.vercel.app";
    const resetPasswordUrl = `${frontendBaseUrl.replace(/\/+$/, "")}/reset-password/${resetToken}`;
