...
const TechFeed: React.FC = () => {
  ...
  const fetchNews = async () => {
    try {
      const res = await fetch(`${baseUrl}/api/v1/techfeed/latest`);
      ...
      if (data.data) {
  formatted = data.data.map((item: any, index: number) => ({
    id: String(index),
    title: item.title,
    ...
    category: 'tech'
      ...
      // ✅ IMPORTANT: update global state (NOT setArticles)
      //dispatch({ type: 'SET_NEWS', payload: formatted });
      ...
      try {
        ...
        if (fallbackData.results) {
          const formatted = fallbackData.results.slice(0, 20).map((item: any, index: number) => ({
            id: String(index),
            title: item.title,
            summary: item.description || "No description",
            url: item.link,
          ...
          //dispatch({ type: 'SET_NEWS', payload: sampleNewsItems });
