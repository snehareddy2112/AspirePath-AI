    ...
    const fetchNotifications = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Map backend notification format to frontend format if needed
          setNotifications(
            data.map((n: any) => ({
              id: n._id,
              type: n.type || "info",
          ...
          {filteredNotifications.length > 0 ? (
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredNotifications.map((notification) => (
                <div
                  key={notification.id}
                  className={`group p-4 border-l-4 ${getPriorityColor(
                            ...
                            <span
                              className={`text-xs px-2 py-1 rounded-full ${
                                state.darkMode
                                  ...
                                  : "bg-gray-100 text-gray-600 group-hover:bg-gray-700 group-hover:text-white"
                              }`}
                            >
