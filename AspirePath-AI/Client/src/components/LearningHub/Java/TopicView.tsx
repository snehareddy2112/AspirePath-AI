...
import { javaModules } from './moduleContent';
...
const TopicView: React.FC<TopicViewProps> = ({ moduleId, topicId, onBack, onTopicChange }) => {
  ...
  const moduleContent = javaModules[moduleId as keyof typeof javaModules];
  ...
  const handleSaveNotes = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/java/notes/${topicId}`, {
        method: 'POST',
        headers: {
  ...
  const handleRating = async (newRating: number) => {
    setRating(newRating);
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/java/reviews/${topicId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleProgressUpdate = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/java/progress/${topicId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
