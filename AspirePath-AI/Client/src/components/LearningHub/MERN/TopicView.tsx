...
const TopicView: React.FC<TopicViewProps> = ({ moduleId, topicId, onBack, onTopicChange }) => {
  ...
  const handleSaveNotes = async () => {
    try {
      const response = await fetch(`/api/v1/learning/mern/notes/${topicId}`, {
        method: 'POST',
        headers: {
  ...
  const handleRating = async (newRating: number) => {
    setRating(newRating);
    try {
      const response = await fetch(`/api/v1/learning/mern/reviews/${topicId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleProgressUpdate = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/learning/mern/progress/${topicId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
