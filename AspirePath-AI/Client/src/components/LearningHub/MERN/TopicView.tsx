...
const TopicView: React.FC<TopicViewProps> = ({ moduleId, topicId, onBack, onTopicChange }) => {
  ...
  const handleSaveNotes = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/mern/notes/${topicId}`, {
        method: 'POST',
        headers: {
  ...
  const handleRating = async (newRating: number) => {
    setRating(newRating);
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/mern/reviews/${topicId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleProgressUpdate = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/mern/progress/${topicId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        ...
        <div className={`${state.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg p-6 border mb-6`}>
          ...
          <div className="w-full">
            ...
            <div className={`w-full h-3 rounded-full overflow-hidden ${
              ...
              <div 
                className="h-full bg-gradient-to-r from-green-500 to-teal-600 transition-all duration-500 ease-out"
                ...
                {moduleContent.topics.map((topic, index) => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-lg transition-all duration-200 border-2 ${
                      topic.id === topicId
                        ? 'bg-gradient-to-r from-green-500 to-teal-600 text-white border-transparent shadow-lg transform scale-105'
                        : state.darkMode
              ...
              <div className={`${state.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg p-6 border mb-6`}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <h1 className={`text-3xl font-bold mb-3 bg-gradient-to-r from-green-600 to-teal-600 bg-clip-text text-transparent`}>
                      {currentTopic.title}
                    </h1>
                    <p className={`text-lg ${state.darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      Master MERN stack development step by step
                    </p>
                  </div>
                </div>
                  ...
                  <div className={`${state.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg border overflow-hidden`}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button 
                          onClick={() => {
                            ...
                            a.href = url;
                            a.download = `${currentTopic.title.replace(/\s+/g, '_')}${getFileExtension()}`;
                            a.click();
                    ...
                    <div className={`${state.darkMode ? 'bg-gray-900' : 'bg-gray-50'} p-4 overflow-x-auto`}>
                      <pre className={`text-sm leading-relaxed ${state.darkMode ? 'text-gray-300' : 'text-gray-800'}`}>
                        <code className={`language-${moduleId === 'react' ? 'jsx' : moduleId === 'nodejs' || moduleId === 'mongodb' ? 'javascript' : moduleId}`}>
                          {currentTopic.codeExample}
                        </code>
                      </pre>
                    </div>
              ...
              <div className={`${state.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg p-6 border`}>
                ...
                <div className="space-y-4">
                  <textarea
                    ...
                    placeholder="✍️ Take notes while learning MERN stack... Write down key concepts, questions, or insights!"
                    className={`w-full h-40 p-4 rounded-lg border-2 resize-none transition-all focus:ring-2 focus:ring-green-500 focus:border-green-500 ${
                      state.darkMode
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-6 py-2 bg-gradient-to-r from-green-500 to-teal-600 text-white rounded-lg hover:from-green-600 hover:to-teal-700 transition-all duration-200 transform hover:scale-105 shadow-lg"
                    >
                      💾 Save Notes
                    </button>
                  </div>
