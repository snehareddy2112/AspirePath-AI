...
const NotesModal: React.FC<NotesModalProps> = ({ isOpen, onClose, moduleId, moduleTitle }) => {
  ...
  const handleSaveNotes = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/aiml/notes/${moduleId}`, {
        method: 'POST',
        headers: {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className={`${state.darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-6 w-full max-w-2xl mx-4`}>
        ...
        <textarea
          ...
          placeholder="Write your AI/ML notes here..."
        ...
        <div className="flex justify-end gap-3 mt-4">
          ...
          <button
            ...
            disabled={loading}
            className="px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-lg hover:from-purple-600 hover:to-pink-700 disabled:opacity-50"
          >
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {modules.map((module, index) => {
          const moduleContent = aimlModules[module.id as keyof typeof aimlModules];
