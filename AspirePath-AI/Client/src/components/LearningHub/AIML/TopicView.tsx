...
const TopicView: React.FC<TopicViewProps> = ({ moduleId, topicId, onBack, onTopicChange }) => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        // Load progress
        const progressResponse = await fetch(`${baseUrl}/api/v1/learning/aiml/progress/${topicId}`, {
        ...
        // Load notes
        const notesResponse = await fetch(`${baseUrl}/api/v1/learning/aiml/notes/${topicId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (notesResponse.ok) {
        ...
        // Load rating
        const reviewResponse = await fetch(`${baseUrl}/api/v1/learning/aiml/reviews/${topicId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const handleSaveNotes = async () => {
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/aiml/notes/${topicId}`, {
        method: 'POST',
        headers: {
  ...
  const handleRating = async (newRating: number) => {
    setRating(newRating);
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/aiml/reviews/${topicId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleProgressUpdate = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/aiml/progress/${topicId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
              ...
              <div className={`${state.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg p-6 border`}>
                ...
                <div className="space-y-4">
                  <textarea
                    ...
                    placeholder="🤖 Take notes on AI/ML concepts... Write down algorithms, formulas, or insights!"
