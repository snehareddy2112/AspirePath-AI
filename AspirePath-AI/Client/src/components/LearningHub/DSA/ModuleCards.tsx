...
import { PlayCircle, FileText, CheckCircle, X } from 'lucide-react';
...
const NotesModal: React.FC<NotesModalProps> = ({ isOpen, onClose, moduleId, moduleTitle }) => {
  ...
  const handleSaveNotes = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${baseUrl}/api/v1/learning/dsa/notes/${moduleId}`, {
        method: 'POST',
        headers: {
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {modules.map((module, index) => {
          const moduleContent = dsaModules[module.id as keyof typeof dsaModules];
