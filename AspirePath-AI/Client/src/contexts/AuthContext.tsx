...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    const savedAuth = localStorage.getItem("aspirePathAuth") || localStorage.getItem("devElevateAuth");
    if (savedAuth) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem("aspirePathAuth", JSON.stringify(state));
  }, [state]);
...
const verifySignupOtp = async (email: string, otp: string) => {
  ...
  const logout = () => {
    ...
    localStorage.removeItem("aspirePathAuth");
  ...
  const updateProfile = async (data: Partial<User>) => {
    ...
    try {
      ...
      const savedUsers = JSON.parse(
        localStorage.getItem("aspirePathUsers") || localStorage.getItem("devElevateUsers") || "[]"
      );
      ...
      if (userIndex !== -1) {
        ...
        localStorage.setItem("aspirePathUsers", JSON.stringify(savedUsers));
      }
  ...
  const deleteUser = (userId: string) => {
    ...
    localStorage.setItem("aspirePathUsers", JSON.stringify(filteredUsers));
